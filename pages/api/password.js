...
export default async (req, res) => {
  ...
  try {
    ...
    if (user.password.trim().length === 0 && user.signInMethod && user.signInMethod !== 'email') {
      ...
      await User.findOneAndUpdate({ _id }, { password: requested });
      res.status(203).send("Password updated");
    } else if (current && !isLength(password, { min: 6 })) { // If not first time set, current password is required
      ...
      return res.status(404).send("Current password required to update password.");
    }
  } catch (error) {
    console.error(error);
